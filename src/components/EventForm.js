...
const EventForm = () => {
  ...
  const deleteAllOperationLogs = e => {
    e.preventDefault()
    const result = window.confirm('すべての操作ログを本当に削除しますか？')
    if(result){
      dispatch({
  ...
  return (
    <>
      ...
      <form>
        ...
        <button className="btn btn-danger" onClick={delteAllEvents} disabled={state.events.length === 0}>全てのイベントを削除する</button>
